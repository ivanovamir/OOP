...
namespace Lab4
{
    public partial class Form1 : Form
    {
        ...
        private void SelectingPerformingOperation(Operator op)
        {
                ...
                if (operands.Count == 2)
                {
                    ...
                    if (circle != null)
                    {
                        if (Init.Coords_check(circle.x, circle.y, r * 2, r * 2))
                        {
                            ...
                            circle.Draw();
                            comboBox1.Items.Add($"Радиус окружности {circle.name} изменен.");
                        }
                        else
                        {
                            MessageBox.Show("Фигура не может выйти за границы.");
            ...
            else if (op.symbolOperator == 'M')
            {
                if (operands.Count == 3)
                {
                    ...
                    int x = Convert.ToInt32(operands.Pop().value.ToString());
                    string name = operands.Pop().value.ToString();
                    Circle circle = FindByName(name) as Circle;
                    if (circle != null)
                    {
                        if (Init.Coords_check(circle.x + x, circle.y + y, circle.w, circle.h))
                        {
                            circle.MoveTo(x, y);
                            comboBox1.Items.Add($"Окружность {circle.name} успешно перемещена.");
                        }
                        else
                        {
                            MessageBox.Show("Фигура не может выйти за границы.");
            ...
            else if (op.symbolOperator == 'D')
            {
                if (operands.Count == 1)
                {
                    ...
                    if (circle != null)
                    {
                        ...
                        comboBox1.Items.Add($"Окружность {circle.name} удалена.");
                    }
                    else
                    {
                        MessageBox.Show($"Окружность с именем {name} не найдена.");
        ...
        {
            if (!(item == 'I' || item == 'D' || item == 'M' || item == 'C' || item == ',' || item == '(' || item == ')'))
            {
                return true;
            }
            else
